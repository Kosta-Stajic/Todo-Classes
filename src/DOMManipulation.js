...
class DomManipulation {
    ...
    handleFormSubmit(form, dialog, isEditMode = false, taskData) {
        form.addEventListener("submit", (e) => {
            ...
            if (!isEditMode){
                ...
                const taskData = task.addTask(title, description, priority, dueDate)

                this.renderTasks(taskData, mainBar)
                
            } else {
                const existingTask = task.todos.find(todo => todo.taskId === taskData.taskId)
                const oldCardElement = document.querySelector(`.todoCards[data-id="${taskData.taskId}"]`)
               if (existingTask) {
    ...
    taskDone(element, checkbox, taskData) {
        checkbox.addEventListener("change", () => {
            ...
            task.isCompleted(taskData)
            console.log(this.taskManager.todos)
        })
    }
