...
class DomManipulation {
    ...
    handleFormSubmit(form, dialog, isEditMode = false, taskData) {
        form.addEventListener("submit", (e) => {
            ...
            if (!isEditMode){
                const taskData = this.taskManager.addTask(title, description, priority, dueDate)
                this.renderTasks(taskData, mainBar)
                
            } else {
                const existingTask = this.taskManager.todos.find(todo => todo.taskId === taskData.taskId)
